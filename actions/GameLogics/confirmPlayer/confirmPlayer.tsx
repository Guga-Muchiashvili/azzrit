...
export const confirmRequest = async ({ id, tableId }: { id: string | undefined, tableId: string | undefined }) => {
  ...
  const waitingPlayers = table.waitingPlayers ? JSON.parse(table.waitingPlayers) : [];
  const updatedWaitingPlayers = waitingPlayers.filter((player: IUser) => player.id !== id) || [];

  const acceptedTables = user.acceptedTables ? JSON.parse(user.acceptedTables) : [];
